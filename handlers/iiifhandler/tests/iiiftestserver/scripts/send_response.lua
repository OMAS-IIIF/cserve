...
function send_error(status, msg)
    if type(status) == "number" and status ~= 200 and type(msg) == "string" then
        ...
        result = {
            message = "Unknown error. Please report this as a possible bug in a IIIF route."
        }
        http_status = 500
    end
...
function send_success(result)
    if type(result) == "table" then
        local success
        local errormsg
        success, errormsg = server.sendHeader("Content-Type", "application/json")
        if not success then
            print(">>>1 ", errormsg)
        end
        local jsonstr
        ...
        success, errormsg = server.print(jsonstr)
        if not success then
            print(">>>3 ", errormsg)
        end
    else
